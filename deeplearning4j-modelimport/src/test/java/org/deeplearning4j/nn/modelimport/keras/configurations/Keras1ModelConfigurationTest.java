...
public class Keras1ModelConfigurationTest {
    ...
    @Test
    public void lstmTimeDistributedSoftmaxAPITest() throws Exception {
        //forwardModelConfigTest("configs/keras1/lstm_timedistributed_softmax_api_config.json");
	forwardModelConfigTest("/home/egoitz/Tools/time/kkk/kkkapp/weights-improvement-732.hdf5");
	//forwardModelConfigTest("/home/egoitz/Tools/time/kkk/kkkapp/model.h5");
	//forwardModelConfigTest("/home/egoitz/Tools/time/kkk/kkkapp/weights-improvement-732.hdf5");
	//forwardModelConfigTest("/home/egoitz/To2ols/time/timenorm/src/main/resources/org/clulab/timenorm/model/char-3softmax-extra/lstm_models_2features.hdf5");	
    }
    ...
    private void forwardModelConfigTest(String path) throws Exception {
        // ClassPathResource configResource = new ClassPathResource(path, classLoader);
        // ComputationGraphConfiguration config =
	//     new KerasModel().modelBuilder().modelHdf5Filename(path)
	//     .enforceTrainingConfig(true).buildModel().getComputationGraphConfiguration();
	//new KerasModel().modelBuilder().modelJsonInputStream(configResource.getInputStream())
	//  .enforceTrainingConfig(true).buildModel().getComputationGraphConfiguration();
        //ComputationGraph model = new ComputationGraph(config);
	ComputationGraph model = KerasModelImport.importKerasModelAndWeights(path, false);
        //model.init();
	//System.out.println(model.summary());
	INDArray Arr = Nd4j.ones(1,1,5);
	model.setInput(0, Arr);
	model.setInput(1, Arr);
	model.feedForward();
    }
